...
import org.kr.scala.z80.system.{ConsoleDebugger, ConsoleDetailedDebugger, Debugger, DummyDebugger, InputPort, InputPortConstant, InputPortSequential, PortID, RegSymbol, Regs, StateWatcher, Z80System}
...
class OpInOutTest extends AnyFunSuite {
  ...
  test("run OUT (n),A (16-bit IO mode)") {
    //given
    //when
    val systemC=TestUtils.prepareTest(List((Regs.A,0x41)),List((0x0000,0xD3),(0x0001,0x01)))
    //then
    assert(systemC.get.register(Regs.PC)==2)
    assert(systemC.get.output(PortID(1,0x41),0)==0x41)
    assert(systemC.get.output(PortID(111),112)==0)
    //systemC.get.output.print(1)
  }
  test("run OUT (n),A (8-bit IO mode)") {
    //given
    //when
    val systemC = TestUtils.prepareTest8BitIO(List((Regs.A, 0x41)), List((0x0000, 0xD3), (0x0001, 0x01)))
    //then
    assert(systemC.get.register(Regs.PC) == 2)
  ...
  test("run OUT (n),A - multiple (16 bit IO mode - upper 8 bits from register A)") {
    //given
    //when
    ...
    assert(systemC.get.output(PortID(0xFF,0x41),0)==0x41)
    assert(systemC.get.output(PortID(0xFF,0x42),0)==0x42)
    assert(systemC.get.output(PortID(0xFF,0x43),0)==0x43)
    assert(systemC.get.output(PortID(111),112)==0)

    //systemC.get.output.print(0xFF)
  }
  test("run OUT (n),A - multiple (8 bit IO mode)") {
    //given
    //when
    val systemC = TestUtils.prepareTest8BitIO(List(),
      List(
        (0x0000, 0x3E), (0x0001, 0x41), // LD A,'A'
        (0x0002, 0xD3), (0x0003, 0xFF), // OUT (0xFF),A
        (0x0004, 0x3E), (0x0005, 0x42), // LD A,'B'
  ...
  test("run OUT (C),r - multiple (16 bit IO mode - upper 8 bits from register B)") {
    //given
    //when
    val systemC=TestUtils.prepareTest(List(),
      List(
    ...
    assert(systemC.get.output(PortID(0x80),0)==0x41) // B was 0 before the first OUT operation
    assert(systemC.get.output(PortID(0x80,0x42),0)==0x42)
    assert(systemC.get.output(PortID(0x80,0x42),1)==0x43)
    assert(systemC.get.output(PortID(0x80,0x42),2)==0x44)
    assert(systemC.get.output(PortID(0x80,0x42),3)==0x45)
    assert(systemC.get.output(PortID(0x80,0x42),4)==0x46)

    //systemC.get.output.print(0xFF)
  }
  test("run OUT (C),r - multiple (8 bit IO mode)") {
    //given
    //when
    val systemC = TestUtils.prepareTest8BitIO(List(),
      List(
  ...
  def prepareTestWithInput8BitIO(regList: List[(RegSymbol, Int)], memList: List[(Int, Int)], port: PortID, inputPort: InputPort,
                           steps: Int = 1): StateWatcher[Z80System] = {
    val blank = Z80System.blank8BitIO.attachPort(port, inputPort)
    TestUtils.prepareTestWith(StateWatcher(blank), regList, memList, steps)
  }
  ...
  test("run IN A,(n) (16 bit IO mode - upper 8 bits from register A)") {
    ...
    val systemC=prepareTestWithInput(List((Regs.A,0x11)),
      ...
      ), PortID(0x40,0x11),new InputPortConstant(0xAB))
    //then
    assert(systemC.get.register(Regs.PC)==2)
  ...
  test("run IN A,(n) (8 bit IO mode)") {
    //given
    //when
    val systemC = prepareTestWithInput8BitIO(List((Regs.A,0x11)),
      List(
        (0x0000, 0xDB), (0x0001, 0x40), // IN A,(0x40)
  ...
  test("run IN A,(C) (16 bit IO mode - upper 8 bits from register B)") {
    ...
    val systemC=prepareTestWithInput(List((Regs.B, 0x99),(Regs.C,0x20)),
      ...
      ),PortID(0x20,0x99), new InputPortSequential(0xF0,2,0,0x0F),
      4)
    //then
  ...
  test("run IN A,(C) (8 bit IO mode)") {
    //given
    //when
    val systemC = prepareTestWithInput8BitIO(List((Regs.B, 0x99),(Regs.C, 0x20)),
      List(
        (0x0000, 0xED), (0x0001, 0x50), // IN D,(C)
        (0x0002, 0xED), (0x0003, 0x58), // IN E,(C)
        (0x0004, 0xED), (0x0005, 0x60), // IN H,(C)
