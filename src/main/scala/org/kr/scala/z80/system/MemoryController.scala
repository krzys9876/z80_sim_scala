...
object MemoryController {
  ...
  def poke: (Int, Int) => Memory => MemoryController = (address, value) => memory =>
    MemoryController(memory.replaceAt(address, value))
  def pokeMulti: (Int, Vector[Int]) => Memory => MemoryController = (address, values) => memory =>
    MemoryController(memory.replaceAt(address, values))
  def loadHexLine:String => Memory => MemoryController = line => memory =>
    MemoryController(memory.loadHexLine(line))
  def loadHexLines:List[String] => Memory => MemoryController = lines => memory =>
    MemoryController(memory.loadHexLines(lines))
  def lockTo: Int => Memory => MemoryController = upperAddressExcl => memory =>
    MemoryController(memory.lockTo(upperAddressExcl))
