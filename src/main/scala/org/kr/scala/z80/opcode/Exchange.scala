...
object Exchange extends OperationSpec {
  ...
  val exchangeListMap: Map[List[OpCode],List[ExchangeLocationBase]] = Map(
    ...
    List(OpCode(0x08, OpCode.ANY)) -> List(new ExchangeLocation("AF","AF1")),
    List(OpCode(0xD9, OpCode.ANY)) -> List(new ExchangeLocation("BC","BC1"),
      new ExchangeLocation("DE","DE1"),new ExchangeLocation("HL","HL1")),
    //register with memory at SP
    List(OpCode(0xE3, OpCode.ANY)) -> List(new ExchangeLocationIndirect("SP","HL")),
