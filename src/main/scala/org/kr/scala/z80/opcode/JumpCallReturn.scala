...
object JumpCallReturn extends OperationSpec{
  ...
  val conditionListMap: Map[List[OpCode],JumpCondition] = Map(
    ...
    List(OpCode(0xC2),OpCode(0x20),OpCode(0xC4),OpCode(0xC0))->JumpCondition(Flag.Z,value=false),
    List(OpCode(0xEA),OpCode(0xEC),OpCode(0xE8))->JumpCondition(Flag.P,value=true),
    List(OpCode(0xE2),OpCode(0xE4),OpCode(0xE0))->JumpCondition(Flag.P,value=false),
    List(OpCode(0xFA),OpCode(0xFC),OpCode(0xF8))->JumpCondition(Flag.S,value=true),
    List(OpCode(0xF2),OpCode(0xF4),OpCode(0xF0))->JumpCondition(Flag.S,value=false)
  )
