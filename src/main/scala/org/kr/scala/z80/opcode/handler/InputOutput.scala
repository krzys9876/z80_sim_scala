...
import org.kr.scala.z80.system.{Debugger, DummyChange, InputRefreshChange, OutputChange, PortID, SystemChange, Z80System}
...
object InputOutput extends OpCodeHandler {
  override def handle(code: OpCode)(implicit system: Z80System, debugger:Debugger): (List[SystemChange], Int, Int) = {
    ...
    val chgList = actualCode.operation.handle(system, PortID(port), actualCode.source)
    (chgList, actualCode.size, actualCode.t)
  }
}

sealed abstract class InOutOperation(val name:String) {
  def handle(system:Z80System,port:PortID,location:Location)(implicit debugger:Debugger):List[SystemChange]
}

object InOutOpType {
  case object In extends InOutOperation("IN") {
    override def handle(system:Z80System, port:PortID, location:Location)(implicit debugger:Debugger):List[SystemChange]= {
