...
sealed abstract class InOutOperation(val name:String) {
  def handle(system:Z80System,port:PortIDWithUpper,location:Location)(implicit debugger:Debugger):List[SystemChange]
}

object InOutOpType {
  case object In extends InOutOperation("IN") {
    override def handle(system:Z80System, port:PortIDWithUpper, location:Location)(implicit debugger:Debugger):List[SystemChange]= {
