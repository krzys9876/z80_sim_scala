...
abstract class Location(val reg:RegSymbol, val immediate:OptionInt, val offsetPC:OptionInt, val addressReg:RegSymbol,
...
object Location {
  ...
  def copy(loc:LocationBase):Location=Location(loc.reg,loc.immediate,loc.offsetPC,loc.addressReg,loc.directOffset,loc.indirectOffset2Compl,loc.isWord)
}*/

class IncorrectLocation(message : String) extends Exception(message) {}

case class EmptyLocation() extends Location(Regs.NONE,AnyInt,AnyInt,Regs.NONE,AnyInt,AnyInt) {
  override lazy val label:String="empty"
}

case class RegisterLocation(register:RegSymbol)
  extends Location(register,AnyInt,AnyInt,Regs.NONE,AnyInt,AnyInt,false) {
  override lazy val label:String=reg.toString
}

case class ImmediateLocation(immediateValue:Int,override val isWord:Boolean=false)
  extends Location(Regs.NONE,IntValue(immediateValue),AnyInt,Regs.NONE,AnyInt,AnyInt,isWord) {
  override lazy val label:String=f"${immediate()}$isWordString"
}

case class IndirectAddrLocation(offset:Int, override val isWord:Boolean=false)
  extends Location(Regs.NONE,AnyInt,IntValue(offset),Regs.NONE,AnyInt,AnyInt,isWord) {
...
case class RegisterAddrLocation(register:RegSymbol, override val isWord:Boolean=false)
  extends Location(Regs.NONE,AnyInt,AnyInt,register,AnyInt,AnyInt,isWord) {
...
case class RegisterAddrDirOffsetLocation(register:RegSymbol, offset:Int, override val isWord:Boolean=false)
  extends Location(Regs.NONE,AnyInt,AnyInt,register,IntValue(offset),AnyInt,isWord) {
  override lazy val label:String=f"(${addressReg.toString}+0x${directOffset()}%02X)$isWordString"
}

case class RegisterAddrIndirOffsetLocation(register:RegSymbol, offset:Int, override val isWord:Boolean=false)
  extends Location(Regs.NONE,AnyInt,AnyInt,register,AnyInt,IntValue(offset),isWord) {
