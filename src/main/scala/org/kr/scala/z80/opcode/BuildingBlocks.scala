...
trait DestinationIFF extends OpCodeDestLocation {override val destination:Location=RegisterLocation(Regs.IFF)}
trait DestinationIM extends OpCodeDestLocation {override val destination:Location=RegisterLocation(Regs.IM)}
...
trait Arith8bDaa extends OpCodeArithmetic8b with SourceDestA {override val operation:ArithmeticOperation=Daa}
...
trait TransferOnce extends TransferRepeatable {override val repeat:Boolean=false}
trait TransferRepeat extends TransferRepeatable {override val repeat:Boolean=true}
